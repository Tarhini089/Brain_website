<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            ...
            <div class="mt-3 space-y-1">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <x-dropdown-link :href="route('logout')"
                        onclick="event.preventDefault(); this.closest('form').submit();"
                        class="text-xs px-2 py-1 rounded-sm hover:bg-red-700 transition duration-150 ease-in-out">
                        {{ __('Log Out') }}
                    </x-dropdown-link>
                </form>
            </div>
    ...
    <div x-data="{ showModal: false }">
        <button @click="showModal = true"
                class="mt-2 text-sm bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">
            Add Country
        </button>

        <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50">
            <div class="bg-white p-6 rounded shadow-md w-full max-w-md">
                ...
                <form method="POST" action="{{ route('countries.store') }}">
                    ...
                    <div class="mb-4 flex items-center">
                        <input type="checkbox" name="is_inactive" id="is_inactive" class="mr-2">
                        <label for="is_inactive" class="text-sm text-gray-700">Is Inactive</label>
                    </div>
